(ns id_quantique.main
  "Kotodama WASM entrypoint for the id_quantique clean-room actor (L4) — Clojure port.
  ...
  facts (`id_quantique.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "id_quantique")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"id_quantique.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "id_quantique-compat" :tier tier :entities entities} 200])
...
(defn start! [] :id_quantique-compat/ready)
